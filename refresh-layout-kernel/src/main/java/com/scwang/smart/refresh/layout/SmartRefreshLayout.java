...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            /**
             * 1 如果第一个View 是RefreshComponent 走第二个if indexContent = 0 contentLevel = 1
             * 2 接着第二个View 是contentView 走第一个if indexContent = 1 contentLevel =2
             * 3 接着第三个View 是是RefreshComponent 没有符合条件的
             */

            /**
             * 如果三个View 都不是RefreshComponent 之后走第一个if indexContent = 0 contentLevel = 2
             *
             * 如果两个View isContentView 都能通过 那么俩个view 都会走第一个if 那么结果就是 indexContent = 1 contentLevel = 2
             * 如果两个View isContentView 第一个通过 第二个不通过 结果是indexContent = 0 contentLevel = 2 那么第二个View 会被转换Footer
             * 如果两个View isContentView 第二个通过 第一个不通过 indexContent = 1 contentLevel = 2 indexHeader = 0
             * 如果两个View isContentView 都不通过 indexContent = 1 第一个还是会转化为header
             *
             * 如果是三个View 相同结果 总之最后 第一个是header 第二个是content 第三个是footer 即使content 不能滑动
             */
        ...
        /**
         * 综上所述 indexContent 只能是 0  或者 1
         *
         * 如果三个View  那么就是 0 1 2 分别是header content footer
         *
         * 如果是两个View 两个Vi
         */
    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {
            ...
            /*mRefreshContent 这玩意 一般是不会为null*/
            ...
            /*只有当没有子View 的时候 mRefreshContent 为null*/
            if (mRefreshContent == null) {
